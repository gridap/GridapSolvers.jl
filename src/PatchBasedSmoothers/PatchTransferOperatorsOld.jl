...
"""
    struct PatchProlongationOperatorOld end

A `PatchProlongationOperatorOld` is a modified prolongation operator such that given a coarse
solution `xH` returns 
...
"""
struct PatchProlongationOperatorOld{R,A,B,C}
  sh :: A
  PD :: B
  ...
  function PatchProlongationOperatorOld{R}(sh,PD,lhs,rhs,is_nonlinear,caches) where R
...
@doc """
    function PatchProlongationOperatorOld(
      lev :: Integer,
      sh  :: FESpaceHierarchy,
...
Returns an instance of `PatchProlongationOperatorOld` for a given level `lev` and a given 
...
"""
function PatchProlongationOperatorOld(lev,sh,PD,lhs,rhs;is_nonlinear=false)
  ...
  R = typeof(Val(redist))
  return PatchProlongationOperatorOld{R}(sh,PD,lhs,rhs,is_nonlinear,caches)
end
...
function MultilevelTools.update_transfer_operator!(op::PatchProlongationOperatorOld,x::Union{AbstractVector,Nothing})
...
function LinearAlgebra.mul!(y::AbstractVector,A::PatchProlongationOperatorOld{Val{false}},x::AbstractVector)
...
function LinearAlgebra.mul!(y::PVector,A::PatchProlongationOperatorOld{Val{true}},x::Union{PVector,Nothing})
  cache_refine, cache_patch, cache_redist = A.caches
  model_h, Uh, fv_h, dv_h, UH, fv_H, dv_H = cache_refine
  fv_h_red, dv_h_red, Uh_red, model_h_red, glue, cache_exchange = cache_redist
  Ph, Ap_ns, Ap, duh, dxp, rp = cache_patch
...
function setup_patch_prolongation_operators_old(sh,lhs,rhs,qdegrees;is_nonlinear=false)
  map(view(linear_indices(sh),1:num_levels(sh)-1)) do lev
    ...
    PatchProlongationOperatorOld(lev,sh,PD,lhs_i,rhs_i;is_nonlinear)
  end
end
...
# PatchRestrictionOperatorOld

struct PatchRestrictionOperatorOld{R,A,B}
  Ip :: A
  rhs :: Union{Function,Nothing}
  caches :: B

  function PatchRestrictionOperatorOld{R}(
    Ip::PatchProlongationOperatorOld{R},
    rhs,
    caches
  ) where R
    A = typeof(Ip)
...
function PatchRestrictionOperatorOld(lev,sh,Ip,rhs,qdegree;solver=LUSolver())
  ...
  redist = has_redistribution(sh,lev)
  R = typeof(Val(redist))
  return PatchRestrictionOperatorOld{R}(Ip,rhs,caches)
end

function MultilevelTools.update_transfer_operator!(op::PatchRestrictionOperatorOld,x::Union{PVector,Nothing})
  # Note: Update is done in the prolongation operator, with which we share the cache
  nothing
end

function setup_patch_restriction_operators_old(sh,patch_prolongations,rhs,qdegrees;kwargs...)
  map(view(linear_indices(sh),1:num_levels(sh)-1)) do lev
    qdegree = isa(qdegrees,Vector) ? qdegrees[lev] : qdegrees
    cparts = get_level_parts(sh,lev+1)
    if i_am_in(cparts)
    ...
    PatchRestrictionOperatorOld(lev,sh,Ip,rhs_i,qdegree;kwargs...)
  end
end

function LinearAlgebra.mul!(y::AbstractVector,A::PatchRestrictionOperatorOld{Val{false}},x::AbstractVector)
...
function LinearAlgebra.mul!(y::Union{PVector,Nothing},A::PatchRestrictionOperatorOld{Val{true}},x::PVector)
  cache_refine, cache_patch, cache_redist = A.caches
  model_h, Uh, VH, Mh_ns, rh, uh, assem, dΩhH = cache_refine
  Ph, Ap_ns, Ap, duh, dxp, rp = cache_patch
  fv_h_red, dv_h_red, Uh_red, model_h_red, glue, cache_exchange = cache_redist
