...
B = VectorValue(0.,1.,1.)
crossB(u::CellField) = Operation(crossB)(u)
crossB(u::VectorValue{2}) = VectorValue(-u[2],u[1])
crossB(u::VectorValue{3}) = u×B

fB(x) = (Dc == 2) ? u_exact(x) : (u_exact(x) - (u_exact(x)⋅B)*B)
f(x) = -Δ(u_exact)(x) + ∇(p_exact)(x) + fB(x)
...
Uh = FESpace(model,reffe_u)
UH = FESpace(cmodel,reffe_u)

B = VectorValue(0.,0.,1.)
uh = zero(Uh)
uH = interpolate(x -> Point(0.,0.,sin(sum(x))), UH)
